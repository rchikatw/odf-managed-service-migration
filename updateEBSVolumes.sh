...
do
  ...
  keyName=$(aws ec2 describe-volumes --volume-id $volumeID --filters Name=tag:kubernetes.io/created-for/pvc/namespace,Values=openshift-storage  --region $region --query "Volumes[*].Tags" --output json $awsProfile | jq .[] | jq -r '.[]| select (.Value == "owned")|.Key')
  backupKeyName=${keyName##*/}
  aws ec2 delete-tags --tags Key=$keyName --resources $volumeID --region $region $awsProfile

  #Update the tag with correct key
  keyName="kubernetes.io/cluster/"$(ocm describe cluster $1 --json | jq -r '.infra_id')
  aws ec2 create-tags --tags Key=$keyName,Value=owned --resources $volumeID --region $region $awsProfile

  nameValue=$(aws ec2 describe-volumes --volume-id $volumeID --filters Name=tag:kubernetes.io/created-for/pvc/namespace,Values=openshift-storage  --region $region --query "Volumes[*].Tags" --output json $awsProfile | jq .[] | jq -r '.[]| select (.Key == "Name")|.Value')
  restoreKeyName=${keyName##*/}
  restoreValue=${nameValue/$backupKeyName/$restoreKeyName}
  aws ec2 create-tags --tags Key=Name,Value=$restoreValue --resources $volumeID --region $region $awsProfile

  aws ec2 create-tags --tags Key=$namespaceKey,Value=$dfOfferingNamespace --resources $volumeID --region $region $awsProfile
  ...
  if [[ "$claimName" == *"rook-ceph-mon"* ]]; then
    aws ec2 modify-volume --volume-type gp3 --volume-id $volumeID --region $region $awsProfile
  elif [[ "$claimName" == *"default"* ]]; then
    aws ec2 modify-volume --volume-type gp3 --iops 12000 --throughput 250 --volume-id $volumeID --region $region $awsProfile
  fi
done
